...
class PraktekScheduleController extends GetxController {
  ...
  void onInit() {
    ...
    doctorId.value = Get.find<AuthController>().my_doctor_id.value;
    getDoctor(doctorId);
  }
  ...
  void removeTimeSlot(id) async {
    ...
    var urlCekPre = Uri.parse('$apiUrl/api/doctor-availabilities/${id}');

    var responseCekPre = await http.delete(urlCekPre, headers: headers);
    getDoctorSchedule(Get.find<AuthController>().my_doctor_id.value);
  }

  void addTimeSlot(TimeOfDay time) async {
    var token = await Get.find<AuthController>().getToken();
    Map<String, String> headers = {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
    ...
    final dt = DateTime(
        ...
        time.hour,
        time.minute);
    final et = dt.add(const Duration(minutes: 20));
    var urlCekPre = Uri.parse(
        '$apiUrl/api/doctor-availabilities??filters[doctor][id][\$eq]=${Get.find<AuthController>().my_doctor_id.value}&sort=start&filters[start][\$eq]=${dt.toUtc().toString()}');

    var responseCekPre = await http.get(urlCekPre, headers: headers);
    var dataPre = jsonDecode(responseCekPre.body)['data'];
    print(dataPre);
    if (dataPre.length > 0) {
      print('ALREADY THERE');
    } else {
      var submitData = {
        "data": {
          "start": dt.toUtc().toString(),
          "end": et.toUtc().toString(),
          "available": true,
          "doctor": Get.find<AuthController>().my_doctor_id.value,
          "price": doctor['attributes']['rate_video']
        }
      };
      print(submitData);
      var urlCek = Uri.parse('$apiUrl/api/doctor-availabilities');

      var responseCek = await http.post(urlCek,
          headers: headers, body: jsonEncode(submitData));
