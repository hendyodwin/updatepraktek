...
class BookingDetailController extends GetxController {
  ...
  RxBool midtransCoreApi = false.obs;
  ...
  void getRemoteConfig() async {
    ...
    await remoteConfig.ensureInitialized().then((value) async {
      print("remote value -> ${await remoteConfig.getBool("midtrans_core")}");
      midtransCoreApi.value = remoteConfig.getBool('midtrans_core');
    });

    print(
        'Remote Config Midtrans ===> ${remoteConfig.getBool('midtrans_core')}');
  }

  // void initSDK() async {
  //   _midtrans = await MidtransSDK.init(
  //     config: MidtransConfig(
  //       clientKey: "Mid-client-py4vZOUpuLZmtc90",
  //       merchantBaseUrl: "https://app.midtrans.com/snap/v1/",
  //       colorTheme: ColorTheme(
  //         colorPrimary: kPrimary,
  //         colorPrimaryDark: kPrimary,
  //         colorSecondary: kSecondary,
  //       ),
  //     ),
  //   );
  ...
  //     print('SNAP RESULT AFTER PAYMENT: ${result.toJson()}');
  ...
  void placeOrder() async {
    ...
    try {
      ...
      if (midtransCoreApi.value) {
        ...
        Get.find<MainController>().trackPaymentMixPanel('MidTransSnap');
        print('==> Snapping');
        var dataPackageQris = {
          "gross_amount": orderAmount,
          "order_id": orderId.toString()
        ...
        var urlPostSnap = Uri.parse('${apiMidtrans}/snap');
        var responsePostGoPay = await http.post(urlPostSnap,
            headers: headers, body: jsonEncode(dataPackageQris));
        final snapToken = responsePostGoPay.body;
        final snapDetail = jsonDecode(responsePostGoPay.body);

        debugPrint('SNAP TOKEN ==> ${snapToken.toString()}');

        await launchUrl(Uri.parse(snapDetail['redirect_url']),
        ...
        //   token: snapToken.toString(),
        // );
      }
    } on Exception catch (e) {
      debugPrint('SNAP ERROR ==> ${e.toString()}');
      isLoading.value = false;
    }
